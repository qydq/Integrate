...
关闭线程可以调用Thread.interrupt
(1) wait阻塞，释放同步锁，使得其他线程可以使用同步控制块或者方法。我们可以捕获Execption，然后关闭

(2)如果是运行中的，线程不受影响，我们可以标记状态，设置为True，然后在适当我的位置，通过Thread.isInterrupt（）来做处理。
还可以通过，shutdownNow,shutdown()方法释放同步锁。
